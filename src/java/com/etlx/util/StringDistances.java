...
public class StringDistances {
	...
	*/		
    ...
    public static double compareName(String s1, String s2) {
        Vector<String> st1 = StringDistances.tokenize(Synonym.synonymize(s1));
        Vector<String> st2 = StringDistances.tokenize(Synonym.synonymize(s2));
        double maxst1[] = new double[st1.size()];
        double avg1 = 0, avg2 = 0;
        Arrays.fill(maxst1, 1);
        double maxst2[] = new double[st2.size()];
        Arrays.fill(maxst2, 1);
        for (int i = 0; i < st1.size(); i++) {
            for (int j = 0; j < st2.size(); j++) {
                double dist = StringDistances.ngramDistance(st1.get(i), st2.get(j));
                if (dist < maxst1[i]) {
                    maxst1[i] = dist;
                }
                if (dist < maxst2[j]) {
    ...
    public static double compareTableName(String s1, String s2) {
        s1 = s1.replaceFirst("WELL_", "");
        s2 = s2.replaceFirst("WELL_", "");
        return StringDistances.ngramDistance(Synonym.synonymize(s1), Synonym.synonymize(s2));
    }
